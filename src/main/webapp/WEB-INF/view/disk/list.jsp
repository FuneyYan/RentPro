                            ...
                            <c:forEach items="${diskList}" var="disk">
                              <tr>
                                ...
                                <td>
                                    <c:choose>
                                        <c:when test="${disk.type=='file'}">
                                            <a href="/netdisk/download?id=${disk.id}">${disk.sourcename}</a>
                                        </c:when>
                                        <c:otherwise>
                                            <a href="/netdisk?path=${disk.id}">${disk.sourcename}</a>
                                        </c:otherwise>
                                    </c:choose>
                                </td>
                                <td>${disk.size}</td>
...
<script>
    ...
    $("#newFolder").click(function () {
       var fid=${fid};
        layer.prompt({title:"请输入文件夹名称"},function (text,index) {
            layer.close(index);
            $.post("/netdisk/folder/new",{"fid":fid,"sourcename":text}).done(function (resp) {
    ...
    $(function () {
        ...
        $(".remove").click(function () {
            ...
            layer.confirm("确定要删除该文件?",function (index) {
                layer.close(index);
                $.get("/netdisk/del/"+id).done(function (resp) {
                    if(resp.status=='success'){
                        window.history.go(0);
                        layer.msg("删除成功");
                    }else{
